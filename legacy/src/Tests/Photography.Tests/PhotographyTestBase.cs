...
namespace Photography.Tests
{
    public abstract class PhotographyTestBase : AbpIntegratedTestBase<PhotographyTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected PhotographyTestBase()
        {
        ...
        protected void UsingDbContext(Action<PhotographyDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<PhotographyDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<PhotographyDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<PhotographyDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<PhotographyDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<PhotographyDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<PhotographyDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<PhotographyDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<PhotographyDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<PhotographyDbContext, Task<T>> func)
        {
            T result;
